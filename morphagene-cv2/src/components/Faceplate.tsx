  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", padding: "8px 10px 10px" }}>
      <svg width={180} height={395} style={{ overflow: "visible" }}>
        {/* Panel body */}
        <rect x={2} y={2} width={176} height={391} rx={4} fill={T.surface2} stroke={T.border} strokeWidth={1.5} />

        {/* Mounting holes */}
        {[12, 383].map(y => (
          <circle key={y} cx={90} cy={y} r={4.5} fill={T.dim} stroke={T.border} strokeWidth={1} />
        ))}

        {/* Module name */}
        <text x={90} y={28} textAnchor="middle" fontFamily="'DM Mono',monospace" fontSize={7}
        ...
        {ROWS.map(({ id, ky, jy }) => {
          ...
          return (
            <g key={id} style={{ cursor: "pointer" }} onClick={() => onSelect(id)}>
              {/* Row highlight */}
              <rect x={10} y={ky - 20} width={155} height={47} rx={3}
                fill={isAct ? col + "14" : "transparent"}
                stroke={isAct ? col + "38" : "transparent"} strokeWidth={1} />

              {/* Knob */}
              <circle cx={47} cy={ky} r={12} fill={T.dim} stroke={col} strokeWidth={isAct ? 1.8 : 0.8} />
              <line
                x1={47} y1={ky}
                x2={47 + Math.sin(ang) * 9} y2={ky - Math.cos(ang) * 9}
                stroke={col} strokeWidth={isAct ? 2 : 1.2} strokeLinecap="round"
              />

              {/* Jack */}
              <circle cx={47} cy={jy} r={6.5} fill={T.surface} stroke={col} strokeWidth={isAct ? 1.8 : 0.8} />
              <circle cx={47} cy={jy} r={2.8} fill={isAct ? col : T.dim} />

              {/* Label */}
              <text x={65} y={ky + 5} fontFamily="'DM Mono',monospace" fontSize={8.5}
                fill={col} fontWeight={isAct ? "600" : "400"} letterSpacing={0}>
                {inp.label}
              </text>

              {/* CV level bar */}
              <rect x={156} y={ky - 11} width={7} height={22} rx={2} fill={T.track} />
              <rect
                x={156} y={ky - 11 + 22 * (1 - norm)}
        ...
        <text x={90} y={370} textAnchor="middle" fontFamily="'DM Mono',monospace"
          fontSize={7} fill={T.muted} opacity={0.6}>
          IN · OUT · REC · CLK
        </text>
        {[28, 47, 66, 85, 104, 123].map((x, i) => (
